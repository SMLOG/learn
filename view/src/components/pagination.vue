<template>
  <div
    :class="[
      `m-pagination ${placement}`,
      { hidden: hideOnSinglePage && total <= pageSize },
    ]"
  >
    <div class="m-pagination-wrap">
      <span class="mr8" v-if="showTotal"
        >共 {{ totalPage }} 页 / {{ total }} 条</span
      >
      <span
        class="u-item"
      ...
      <span
        :class="['u-item', { active: currentPage === 1 }]"
        @click="changePage(1)"
        >1</span
      >
      <span
...
export default {
  ...
  methods: {
    ...
    onForward() {
      this.currentPage =
        this.currentPage - this.pageListNum > 0
          ? this.currentPage - this.pageListNum
          : 1;
    },
    onBackward() {
      this.currentPage =
        this.currentPage + this.pageListNum < this.totalPage
          ? this.currentPage + this.pageListNum
          : this.totalPage;
    },
    jumpPage(jumpNum) {
